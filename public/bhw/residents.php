...
$family_rows = db()->prepare('
    SELECT fm.resident_id, fm.full_name, fm.relationship, fm.date_of_birth, fm.profile_image
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center space-x-3">
                                                <?php if (!empty($r['profile_image'])): ?>
                                                    <img src="<?php echo htmlspecialchars(upload_url($r['profile_image'])); ?>" 
                                                         alt="<?php echo htmlspecialchars($r['first_name'] . ' ' . $r['last_name']); ?>"
                                                         class="w-10 h-10 rounded-lg object-cover border border-gray-200 flex-shrink-0"
                                                         onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                                    <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg flex items-center justify-center flex-shrink-0 hidden">
                                                        <?php 
                                                ...
                                                <?php else: ?>
                                                    <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg flex items-center justify-center flex-shrink-0 text-white font-semibold text-sm">
                                                        <?php 
                                                        $firstInitial = !empty($r['first_name']) ? substr($r['first_name'], 0, 1) : 'R';
                                                        $lastInitial = !empty($r['last_name']) ? substr($r['last_name'], 0, 1) : 'D';
                                                        echo strtoupper($firstInitial . $lastInitial); 
                                                            ...
                                                            <?php foreach ($families_by_resident[$r['id']] as $index => $family): ?>
                                                                <div class="family-member-card-modern bg-white p-5 rounded-xl shadow-md border border-gray-200 hover:shadow-lg hover:border-purple-300 transition-all duration-300 transform hover:-translate-y-1" style="animation-delay: <?php echo $index * 0.1; ?>s">
                                                                    <div class="flex items-start space-x-4">
                                                                        <?php if (!empty($family['profile_image'])): ?>
                                                                            <img src="<?php echo htmlspecialchars(upload_url($family['profile_image'])); ?>" 
                                                                                 alt="<?php echo htmlspecialchars($family['full_name']); ?>"
                                                                                 class="w-14 h-14 rounded-xl object-cover border border-gray-200 flex-shrink-0 shadow-lg"
                                                                                 onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                                                            <div class="w-14 h-14 bg-gradient-to-br from-purple-500 via-indigo-500 to-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg text-white font-semibold text-lg hidden">
                                                                                <?php 
                                                                                $nameParts = explode(' ', $family['full_name']);
                                                                                $firstInitial = !empty($nameParts[0]) ? substr($nameParts[0], 0, 1) : 'F';
                                                                                $lastInitial = !empty($nameParts[count($nameParts) - 1]) ? substr($nameParts[count($nameParts) - 1], 0, 1) : 'M';
                                                                                echo strtoupper($firstInitial . $lastInitial); 
                                                                                ?>
                                                                            </div>
                                                                        <?php else: ?>
                                                                            <div class="w-14 h-14 bg-gradient-to-br from-purple-500 via-indigo-500 to-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg text-white font-semibold text-lg">
                                                                                <?php 
                                                                                $nameParts = explode(' ', $family['full_name']);
                                                                                $firstInitial = !empty($nameParts[0]) ? substr($nameParts[0], 0, 1) : 'F';
                                                                                $lastInitial = !empty($nameParts[count($nameParts) - 1]) ? substr($nameParts[count($nameParts) - 1], 0, 1) : 'M';
